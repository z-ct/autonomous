...
There's no need to go into all of the various transactions that can occur between cars, but, to summarise, at least the following are feasible:

- bribery for passing (e.g. our Sprint example)
- bribery for parking
- override for the above for emergency vehicles
- uber when vehicle is empty
- as well as down the waterfall services like negotiated insurance for the vehicles dependent on their use, and negotiated finance on the above given the user's risk profiles.
...
We suggest that this could be possible by stressing that critical masses are required for the above-mentioned transactions to be possible. Building critical mass is helped along by the regulation that requires that by 2020, all vehicles (not just AVs), be able to communicate with all other vehicles on the road.
...
We will provide this communication network, and the associated transaction verification and clearing and settlement for the transactions.

*Building this protocol will allow for the critical mass required for the transactions above to work*, even if not all cars are autonomous. This will work despite cars being made by different manufacturers. This will be pro-competitive as it will be an open platform (the regulators will be on board). This will also be an independent system, which the AV manufacturers can interact with using APIs.
...
This would follow much the same pattern as the autonomous vehicles, but in a less established and concentrated space.
...
That's where Z-CT fits in. You pay us the cost of the cheapest ticket you see today. We guarantee you a seat and offer you 50% of what we save. We do not purchase the ticket until we get a discounted rate. We can get this discount by having multiple users purchase the same flight and negotiating with the airline. Or, we can get the discount by simply waiting to purchase the tickets until prices go down (a la hopper). This turns our agency model on its head, and, whilst invisible to the consumer, we’d benefit from its resilience.

## Smart fridge use case

You own a smart fridge that keeps track of everything you buy. It also knows when are run out of raspberries which you usually eat when you are stressed out. It is Thursday and you forgot to buy them, but Z-CT never forgets. We know your Friday is busy, we know you'll be stressed, and we raspberry know to buy you raspberries before you do. We already did that purchase for you, buying the same raspberry you usually consume at the best price available. Now you will have them in your fridge whenever you're stressed.
